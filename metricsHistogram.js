...
function fileAdded(path) {
  ...
  const split = path.split(WINDOWS_PATH);
  const filename = split[split.length-1];
  console.log(filename);

  if (filename === 'callng.log.2.status') {
...
async function readWholeFileCallngLog(path) {
  ...
  await readWholeFileCallngLogWithHistogramCounter(path);
  // performance
...
async function readUpdatedFileCallngLog(path) {
  ...
  const meter = meterProvider.getMeter('callng.log.2.status');
...
async function readWholeFileCallngLogWithSyncUpDownCounter(path) {
  ...
  const histogramLog = meter.createHistogram('histogram_counter_example',{
...
//Example of implementing Histogram Counter to count latency
